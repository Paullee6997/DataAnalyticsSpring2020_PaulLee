...
USING Cross Validation function tune() from  e1071
...
svm(formula = Species ~ ., data = iris, method = "C-classification",kernel = "radial", cost = 10, gamma = 0.1)
...
svm(formula = type ~ ., data = spamtrain, cost = bestC, gamma = bestGamma,cross = 10)
...
library("klaR")
data("B3")
Bmod <- svmlight(PHASEN ~., data = df, svm.options = "-c 10 -t 2 -g 0.1 -v 0")
...
x <- balanced.overlap$x
y<-balanced.overlap$y
