...
example for exploring factor analysis 

```{r}
#read the dataset into R variable using the read.csv(file) function
data <- read.csv("/Users/kingg/Desktop/DataAnalytics/dataset_exploratoryFactorAnalysis.csv")
...
library(Hmisc)
AthleticsData <- spss.get("/Users/kingg/Desktop/DataAnalytics/AthleticsData.sav")
...
fit.3.Enzmann <- update(fit.3.promax,factors=3, digits=2, sort=TRUE)
AssignFactorNames(fit.3.Enzmann,fit.3.Enzmann$factors.names)
fit.3.Enzmann
```
...
```{r}
epi <- read.csv("/Users/kingg/Desktop/DataAnalytics/EPI_data.csv")
